...
async function setupPolicy(policyData: string): Promise<number> {
  // Create a new policy
  try {
    console.log("🏛️ Attempting to create institutional RWA policy...");
    const result = await RULES_ENGINE.createPolicy(policyData);
    console.log(`✅ Policy '${result.policyId}' created successfully.`);
    console.log("🔗 All 14 institutional compliance rules now active");
...
async function injectModifiers(
  ...
  try {
    console.log("🔧 Injecting institutional compliance modifiers...");
...
async function main() {
  ...
  console.log("🏛️ Institutional RWA Forte SDK");
  console.log("===============================");
  ...
  if (process.argv[2] == "setupPolicy") {
    ...
    if (!policyJSONFile) {
      policyJSONFile = "policies/institutional-complete.json";
    }
    ...
    const policyJSONFile = process.argv[3] || "policies/institutional-complete.json";
    ...
    console.log("     npx tsx sdk.ts setupPolicy policies/institutional-complete.json");
    console.log("     npx tsx sdk.ts injectModifiers policies/institutional-complete.json src/InstitutionalRWAFRE.sol src/InstitutionalRWA.sol");
    console.log("     # Deploy contract with forge");
    console.log("     npx tsx sdk.ts applyPolicy 1 <deployed-contract-address>");
  }
}
